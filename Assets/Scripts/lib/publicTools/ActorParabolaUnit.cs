...
namespace xy3d.tstd.lib.publicTools
{
	class ActorParabolaUnit
	{
	    ...
	    public ActorParabolaUnit(GameObject _effectGO, Vector3 _startPos, Vector3 _endPos, Vector3 _interval)
	    {
            ...
            isZ = endPos.x != startPos.x;

            if (isZ)
            {
                v.z = (endPos.z - startPos.z) / distance + interval.z * distance * 0.5f;
            }
            else
            {
	    ...
	    public void SetPercent(float value)
	    {
            if (isZ)
            {
                newPos.x = startPos.x + (endPos.x - startPos.x) * value;// (v.x * 2 - interval.x * distance * value) * distance * value * 0.5f;
                newPos.z = startPos.z + (v.z * 2 - interval.z * distance * value) * distance * value * 0.5f; // (endPos.z - startPos.z) * value; 
            }
            else
            {
