        ...
        public void Init(GameObject con, Action call)
        {
			...
			for(i = 0 ; i < damageDrawNum ; i++){
				...
				for(m = 0 ; m < damageUnitNum ; m++){
					...
					unitVec[i, m] = new BattleDamageNumUnit(m);
...
//                                tempMR.material.SetVector("scaleFix" + n.ToString(), tempGroup.GetScaleFix());
        ...
        public BattleDamageNumGroup AddSpeedNum(string _str, Vector3 _pos)
        {
            _pos.y = _pos.y + 2.5f;
            int nowDrawIndex = -1;
        ...
        public void AddDamageNum(string _str, int _color, Vector3 _pos, bool _randomDirection, bool _isFly)
        {
            _pos.y = _pos.y + 2.5f;
			int nowDrawIndex = -1;
            ...
            if(!_isFly){
				...
				int id = SuperTween.Instance.To(unit.nowScale, targetScale, largeTime, setScaleCall, scaleCall);
                ...
                int id = SuperTween.Instance.To(unit.pos.y, unit.pos.y + flyHeight, flyTime, setMoveCall, moveCall);
                SuperTween.Instance.SetTag(id, "battle_tag");
        ...
        public void Small1Over(BattleDamageNumGroup _unit)
        {
            ...
            int id = SuperTween.Instance.DelayCall(waitTimeConst, waitCall);
            SuperTween.Instance.SetTag(id, "battle_tag");
        }
        ...
        public void Small(BattleDamageNumGroup _unit)
        {
			...
			int id = SuperTween.Instance.To(_unit.nowScale, 0, smallTime, setScaleCall, scaleCall);
            SuperTween.Instance.SetTag(id, "battle_tag");
        }
